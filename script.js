...
brett_id[hovednokkel] = [];
let brett_queue = [];
let riktigesteg = [];
let retrieve_recursive = false;
const spille_paa_tur = true;
const kontroller_sjakk = false;
...
let sjekke_sjakk_sin_tur = true;
let brett_id_backup = [];
let PieceIsMoved = false;
let har_vunnet = { white: false, black: false };
let sjekk_valid_move = true;

// Funksjoner
// Små hjelpefunksjoner
function rett_opp_riktige_steg() {
  riktigesteg = toFindDuplicates(riktigesteg, false);
}
        ...
        if (brett_id[hovednokkel][i][j].BoardInfo.BoardPiece[1] === "King") {
          switch (brett_id[hovednokkel][i][j].BoardInfo.BoardPiece[0]) {
            case "white":
              ...
              console.log(kongensplass.white);
              kongensplass.white = [i, j];
              har_vunnet.black = false;
              console.log(har_vunnet);
              break;
            case "black":
              console.log("case black");
              kongensplass.black = [i, j];
              console.log(kongensplass.black);
    ...
    for (let j = 0; j < 8; j++) {
      ...
      brett
        ...
        .addEventListener("click", function () {
          brett_id[nokkelkoe] = brett_id[hovednokkel];
          let aa_pushe = {
            index: [j, i],
          ...
          if (brett_queue.length == 1) {
            ...
            if (PieceIsMoved) {
              ...
              if (kontroller_sjakk ? sjakk() : false) {
                console.log("Du er satt i sjakk");
                if (sjakk(true)) {
...
// flytte bare brikke
function actuallyMoveBoardPiece(BrettIdElFrom, BrettIdElTo, nokkel) {
  if (!!Object.keys(BrettIdElTo).length) {
    sjekk_valid_move = true;
    PieceIsMoved = true;
...
function validmove(BrettIdElFrom, BrettIdElTo, nokkel) {
  ...
  if (spille_paa_tur && er_andre_trekk && sjekk_valid_move) {
    ...
    if (BrettIdElFrom.BoardInfo.BoardPiece[0] == hvem_sin_tur) {
      sjekk_valid_move = false;
      switch (hvem_sin_tur) {
        case "white":
          hvem_sin_tur = "black";
          riktig_spiller = true;
  ...
  if (
    ...
    if (er_andre_trekk) {
      ...
      if (
        dupliatearrboolean(
          riktigesteg.concat([Splitkoordinattilarray(BrettIdElTo.ID.tag)])
        )
      ) {
        lets_move = true;
      }
...
function find_path_med_betingelser(nokkel, a, b, c) {
    ...
    a.betingelser.forEach(function (sjekk_betingelser) {
      switch (sjekk_betingelser.navn) {
        case "None":
          ...
          returnereskal = find_path(nokkel, a, b, c, false, false, false);
              ...
              for (let k = 0; k < (et_trykk ? 2 : 1); k++) {
                ...
                if (
                  ...
                  actuallyMoveBoardPiece(
                    ...
                    brett_id[nokkel][c_nyrook[0]][c_nyrook[1]],
                    nokkel
                  );
                  console.log("heiheihiehiehiehi");
  ...
  console.log(b, c);
  console.log(returnereskal | returnere_skal_dobbel);
...
function sjakk(omkrets = false) {
  let boardinfo = {};
  let det_er_sjakk = false;
  let sette_sjakk = "";
  let annen_sjakk = "";
  let det_er_sjakk_mx = [];
  switch (hvem_sin_tur) {
  ...
  switch (sjekke_sjakk_sin_tur ? hvem_sin_tur : annen_sjakk) {
    case "white":
      sette_sjakk = "black";
      break;
    case "black":
  ...
  let kongestegene = [[0, 0]];
  if (omkrets) {
    kongestegene = spillebrikke[sette_sjakk].King.spillinfo.flytte.path;
  }
  kongestegene.forEach(function (kongesteg) {
    if (
      IndexOnBoard(kongesteg[0] + kongensplass[sette_sjakk][0]) &&
      IndexOnBoard(kongesteg[1] + kongensplass[sette_sjakk][1])
    ) {
      console.log([
        kongesteg[0] + kongensplass[sette_sjakk][0],
        kongesteg[1] + kongensplass[sette_sjakk][1],
      ]);
      det_er_sjakk = false;
      for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
          boardinfo = brett_id["queue"][i][j].BoardInfo;
          if (!!boardinfo.BoardPiece.length) {
            det_er_sjakk = find_path_med_betingelser(
              nokkel,
              spillebrikke[boardinfo.BoardPiece[0]][boardinfo.BoardPiece[1]]
                .spillinfo,
              [[i, j]],
              [
                kongesteg[0] + kongensplass[sette_sjakk][0],
            ...
            console.log(det_er_sjakk);
            det_er_sjakk_mx.push(det_er_sjakk);
            if (det_er_sjakk) {
              console.log("Det er sjakk.");
              det_er_sjakk_mx.push(true);
              break;
            } else {
              console.log("Det er ikke sjakk.");
            }
          }
        }
      }
